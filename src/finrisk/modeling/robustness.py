...
def yearly_metrics(frame:pd.DataFrame,label_col="distress_12m",score_col="score")->dict:
    ...
    for year,g in x.groupby(x["filed"].dt.year):
        if g[label_col].nunique()<2:
            out[str(year)]={"rows":int(len(g)),"positives":int(g[label_col].sum()),"status":"insufficient_classes"}
        else:out[str(year)]=rare_event_metrics(g[label_col],g[score_col])
    return out

def subgroup_metrics(frame:pd.DataFrame,group_col:str,label_col="distress_12m",score_col="score",min_rows=500,min_positives=10)->dict:
    out={}
    for value,g in frame.groupby(group_col,dropna=False):
        key=str(value);positives=int(g[label_col].sum())
        if len(g)<min_rows or positives<min_positives or g[label_col].nunique()<2:
            out[key]={"rows":int(len(g)),"positives":positives,"status":"insufficient_support"}
