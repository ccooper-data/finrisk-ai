...
def test_q2_ytd_duration_is_preserved_when_discrete_missing():
    f=pd.DataFrame({
        "adsh":["q2"],"cik":[1],"form":["10-Q"],"period":pd.to_datetime(["2025-06-30"]),
        "filed":pd.to_datetime(["2025-08-01"]),"feature":["revenue"],"tag":["Revenues"],
        "ddate":pd.to_datetime(["2025-06-30"]),"qtrs":[2],"value":[200.0]
    })
    out=select_period_correct_facts(f)
...
def test_discrete_quarter_is_preferred_over_ytd_when_both_exist():
    ...
    f=pd.DataFrame({
        "adsh":["q2","q2"],"cik":[1,1],"form":["10-Q","10-Q"],
        "period":pd.to_datetime(["2025-06-30","2025-06-30"]),
        "filed":pd.to_datetime(["2025-08-01","2025-08-01"]),
        "feature":["revenue","revenue"],"tag":["Revenues","Revenues"],
        "ddate":pd.to_datetime(["2025-06-30","2025-06-30"]),"qtrs":[1,2],
        "value":[110.0,200.0]
    })
